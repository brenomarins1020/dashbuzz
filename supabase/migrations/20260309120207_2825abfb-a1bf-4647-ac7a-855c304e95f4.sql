...
-- Update trigger to create ALL 11 platforms for new workspaces (4 active, 7 inactive)
...
BEGIN
  ...
  -- Inserir todas as 11 plataformas (4 ativas, 7 desativadas)
  ...
  VALUES
    ...
    (NEW.id, 'YouTube',            'youtube',     4, false),
    (NEW.id, 'Pinterest',          'pinterest',   5, false),
    (NEW.id, 'X / Twitter',        'twitter',     6, false),
    (NEW.id, 'Facebook',           'facebook',    7, false),
    (NEW.id, 'WhatsApp / Canal',   'whatsapp',    8, false),
    (NEW.id, 'Newsletter / Email', 'newsletter',  9, false),
    (NEW.id, 'Site / Landing Page','site',       10, false);
